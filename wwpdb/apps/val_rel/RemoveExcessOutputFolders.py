...
class FindExcessEntries:
    def __init__(self, site_id: Optional[str] = None, dry_run: bool = False) -> None:
        ...
        self.__pdb_entries: Set[str] = set()
        self.__output_pdb_entries: List[str] = []
    ...
    def find_pdb_and_emdb_entries(self) -> None:
        ...
        self.__pdb_entries = pvr.get_all_pdb_entries()

    #        self.__emdb_entries = pvr.get_emdb_entries()

    def get_pdb_output_folder(self) -> str:
        return outputFiles(siteID=self.__site_id).get_pdb_root_folder()

    def get_emdb_output_folder(self) -> str:
        return outputFiles(siteID=self.__site_id).get_emdb_root_folder()

    def find_pdb_output_entries(self) -> None:
        self.__output_pdb_entries = os.listdir(self.get_pdb_output_folder())

    #    def find_emdb_output_entries(self) -> None:
    #        self.__output_emdb_entries = os.listdir(self.get_emdb_output_folder())

    def check_pdb_entries_output_should_exist(self) -> None:
        for entry in self.__output_pdb_entries:
            logger.info(entry)
            if entry not in self.__pdb_entries:
