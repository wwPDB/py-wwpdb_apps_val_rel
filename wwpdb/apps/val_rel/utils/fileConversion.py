...
def convert_cs_file(entry_id, cs_file, model_file, working_dir):
    ...
    if os.path.exists(cs_file):
        ...
        if starToPdbx(entryId=entry_id, starPath=src_cs_file, pdbxPath=cs_cif_file,
                      modelPath=model_file, remediation=True):
            logger.info(f'CS star to cif conversion worked - new cs file: {cs_cif_file}')
