...
class EmailHandler:
    def __init__(self, site_id: Optional[str] = None) -> None:
        ...
        self.__email_interval = vc.val_email_interval
        self.__max_per_interval = vc.val_max_per_interval
    ...
    def send_email_admins(self, msg: str) -> None:
        for admin in self.__admin_list:
            self.send_email(msg, admin)

    def send_email(self, txt: str, recipient: str) -> None:
        if not os.path.exists(self.__dir):
            os.makedirs(self.__dir)
        lock = SoftFileLock(self.__lock_file)
        with lock, shelve.open(self.__shelf_file) as db:  # noqa: S301
            if recipient not in db:
                ...
                # otherwise reset values
                    ...
                    db[recipient] = "%d,%d" % (1, time.time())
        content = f"""
        The Val Rel application at {getSiteId()} threw an exception!
        The following error output was retrieved:
        {txt}"""
        self.email(content, recipient)

    def email(self, content: str, recipient: str) -> None:
